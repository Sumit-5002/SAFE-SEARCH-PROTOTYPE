...
setInterval(async () => {
    ...
    const { totalTimeSpent = 0 } = await chrome.storage.local.get('totalTimeSpent');
    await chrome.storage.local.set({
        totalTimeSpent: totalTimeSpent + timeSpent
    });
}, 60000);

// Function to increment blocked count
...
// Function to increment safe site count
async function incrementSafeCount() {
    const { safeCount = 0 } = await chrome.storage.local.get('safeCount');
    await chrome.storage.local.set({ safeCount: safeCount + 1 });
    
    // Notify popup to update stats
    chrome.runtime.sendMessage({ action: 'safeSiteVisited' });
}
...
async function getStats() {
    try {
        const { blockedDomains, lastUpdate, safeSearchEnabled, aiFilteringEnabled, blockedCount, safeCount, totalTimeSpent } = 
...
chrome.webNavigation.onBeforeNavigate.addListener(async (details) => {
    ...
    try {
        ...
        // Check if it's a safe site
        if (SAFE_SITES.some(site => hostname.includes(site))) {
